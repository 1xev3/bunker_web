...
export default function EventModal({ event, activePlayers, bunker, packSettings, eventSelection, choiceVotes, choicePendingSelection, resolveConfirmations, myPlayerId, send, disabled = false }: Props) {
  ...
  if (event.event_type === 'choice') {
    return <ChoiceEventCard event={event} activePlayers={activePlayers} bunker={bunker} eventSelection={eventSelection} choiceVotes={choiceVotes} choicePendingSelection={choicePendingSelection} myPlayerId={myPlayerId} send={send} disabled={disabled} />;
  }
