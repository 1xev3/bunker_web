...
function randomOf(list, rng = Math.random) {
...
function selectParticipants(spec, room, rng = Math.random) {
  ...
  for (const [role, crit] of Object.entries(spec)) {
    ...
    const picked = randomOf(candidates, rng);
    roleMap[role] = picked;
...
function selectionSuccessChance(strength, diverse, modifier = 0) {
  if (strength <= 0) return 0;
  const cap = diverse ? 100 : SELECTION_MAX_PARTIAL;
  return Math.max(0, Math.min(cap, Math.round(strength * SELECTION_PER_RESOURCE + modifier)));
}
